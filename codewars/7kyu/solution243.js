...
TODO:
split the string into array and convert to numbers
create conditionals
return "tick" if all conditionals run false
*/

const fizzBuzzCuckooClock = t => {
    t = t.split(":").map(e=>+e)
    if(!t[1]) return "Cuckoo ".repeat(t[0] > 12 || !t[0] ? Math.abs(t[0] - 12) : t[0]).trimEnd();
    if(t[1] == 30) return "Cuckoo";
    if(!(t[1] % 3) && !(t[1] % 5)) return "Fizz Buzz";
    if(!(t[1] % 3)) return "Fizz";
    if(!(t[1] % 5)) return "Buzz";
    return "tick";
  };
