...
Explanation: We add two smallest elements (1 + 2), their sum is 3.

Then we add the next smallest number to it (3 + 3) , so the sum becomes 6.
Now the result is greater or equal to 6 , Hence the output is (2) i.e (2) operations are required to do this.
minimumSteps({8 , 9, 4, 2}, 23)  ==> return (3)
Explanation: We add two smallest elements (4 + 2), their sum is 6.
...
Explanation: We add two smallest elements (19 + 17), their sum is 36.
...
return counter if reduced number is greater than or equal to value passed in

*/

const minimumSteps = (n,v) => {
    let counter = 0;
    return n = n.sort((a,b)=>a-b).reduce((a,c) => {
      if(a>=v) return a;
      counter ++;
      return a+c;
    }) >= v && counter;
