...
Will the arguments passed into function always be an array of numbers and a string?
...
TODO:
iterate over array and perform respective operations depending if operating transformations or its inversion
return sorted array in ascending order
*/

const operate = (pcSet, operation) => {
    const I = operation.slice(-1) == "I" ? +(operation.slice(1, -1)) : NaN;
    const n =  Number.isInteger(I) ? I : +(operation.slice(1));  
    return pcSet.map(e => Number.isInteger(I) ? (12 - e + n) % 12 : (e + n >= 12 ? (e + n) % 12 : e + n)).sort((a,b)=>a-b);
};  