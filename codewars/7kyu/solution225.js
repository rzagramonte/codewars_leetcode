...
Will the argument passed into the funtion's parameter always be a string?
Will the string always contain a combination of 'x', 'o', or ' '?
Will the return always be a boolean of true or false?

TODO
iterate over string turned into array
for each:
if index is odd, it flips
if index is even, it stays the same
whitespace never changes
return true for 'x' and ' ', false for 'o'
is every element in the new array true?
*/

const bulbMaze = maze => [...maze].map((e,i)=>{
    switch (e) {
      case 'x': if(!(i%2)) return true;
      case 'x': if(i%2) return false;
      case 'o': if(i%2) return true;
      case 'o': if(!(i%2)) return false;
      default: return true;
      }}).every(e=>e);